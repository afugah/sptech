...
export type SizeGuideStoryData = {
  story?: StoryblokStory<SizeGuide>;
...
export async function fetchSizeGuideStory(locale: string): Promise<SizeGuideStoryData> {
  try {
    ...
    const { data } = await storyblokApi.get(`cdn/stories/content/size-guide`, sbConfig);

    // Cast the story to the correct type
    return {
      story: data?.story as StoryblokStory<SizeGuide>,
      sbConfig,
