...
import { isWishlistEnabled, useInlineHeaderMenu } from '@/src/lib/features';
        ...
        >
          <div className={'bottom-0 flex items-center lg:basis-2/5'}>
            {isOpen ? null : (
              ...
              >
                <BarsIcon
                  role={'button'}
                  className={`pointer h-6 w-6 ${isHomePage && hasHeaderFixed && bgColor ? 'fill-white' : 'fill-black'}`}
                />
              </Button>
            )}
          ...
          >
            <Link href={'/'} aria-label={'Home'} className={'flex items-center space-x-4 md:space-x-6'}>
              <StoreLogo
                className={`h-8 w-auto xs:h-10 sm:h-14 lg:h-16 ${
                    ...
                    <SheetTrigger asChild>
                      <Button variant={'custom'} className={'hidden px-0 py-0 sm:block [&_svg]:size-7'}>
                        <Heart
                          ...
                          strokeWidth={1}
                          color={''}
                          className={`inline-block h-6 w-6 bg-transparent sm:h-8 sm:w-8 ${
                            isHomePage && hasHeaderFixed && bgColor ? 'stroke-white' : 'stroke-black'
                          }`}
                        />
